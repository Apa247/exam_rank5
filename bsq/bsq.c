...
int read_map(FILE *fp, t_map *map)
{
	...
	int i = 0;
	char *line = NULL;
	map->map = calloc(sizeof(char *), map->rows);
	if (!map->map)
		return 0;
	while (i < map->rows && (read = getline(&line, &len, fp)) != -1)
	{
		...
		map->map[i] = calloc(sizeof(char), map->cols + 1);
		if (!map->map[i])
		{
			free(line);
			return 0;
...
int validate_map(t_map *map)
{
	...
	for (int i = 0; i < map->rows; i++)
	{
		...
		{
			char c = map->map[i][j];
			if (c != map->empty && c != map->full && c != map->obstacle)
				return 0;
		}
	}
	return 1;
...
{
	int **dp = calloc(sizeof(int *), map->rows);
	if (!dp)
		return 0;
	for (int i = 0; i < map->rows; i++)
	{
		dp[i] = calloc(sizeof(int), map->cols);
		if (!dp[i])
			return 0;
	}
    ...
    printf("Matriz DP (tamaños de cuadrados):\n");
    for (int i = 0; i < map->rows; i++) {
        for (int j = 0; j < map->cols; j++) {
            printf("%d ", dp[i][j]);
        }
...
int process_map(FILE *fp)
{
	...
	if (!read_map(fp, &map) || !validate_map(&map) || !resolve_map(&map, &square))
	{
		print_error();
		free_map(&map);
		return 0;
